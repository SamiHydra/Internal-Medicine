...
import { FilePlus2, Loader2, Lock, Plus, Save, Send, Trash2 } from 'lucide-react'
...
export function EvaluationFormsPage() {
  ...
  const [contentEdits, setContentEdits] = useState<Record<string, Record<string, { label?: string; helpText?: string }>>>({})
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setDraftEdits(Object.fromEntries(
        data.filter((form) => form.status === 'draft').map((form) => [form.id, toDraftFields(form)]),
      ))
      setLoadError(false)
    } catch {
      setLoadError(true)
  ...
  const stageContent = (formId: string, fieldKey: string, patch: { label?: string; helpText?: string }) => {
    ...
    run(
      ...
      async () => {
        ...
        await updateEvaluationFormContent(client!, form.id, {
          fields: Object.entries(staged).map(([key, patch]) => ({ key, ...patch })),
        })
  ...
  const toggleFieldActive = (form: EvaluationFormDefinition, field: EvaluationFormField, active: boolean) =>
    run(
      `active-${field.id}`,
      async () => {
        await updateEvaluationFormContent(client!, form.id, {
    ...
    <div className="space-y-6 px-4 py-6 md:px-8">
      <section className={panelClass}>
        <SectionHeader
          ...
          description="Every evaluation tool, editable without a developer. Wording and help text apply immediately; structural changes (Maintenance only) create a new version, and past evaluations keep rendering against the version they were answered on."
        />

        {forms === null ? (
          <div className="flex min-h-[240px] items-center justify-center text-[#74777f]">
            <Loader2 className="h-5 w-5 animate-spin" aria-label="Loading forms" />
          </div>
        ) : (
          <div className="mt-5 space-y-8">
            {groups.map((group) => (
              <div key={group.key} className="rounded-[0.4rem] border border-[#e6ecf3] p-5">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    ...
                    <div className="mt-1 flex flex-wrap items-center gap-1.5">
                      <span className="text-xs text-[#74777f]">{group.key}</span>
                      {group.versions.map((version) => (
                        <Badge key={version.id} variant={statusVariant[version.status]}>
                  ...
                  <div className="mt-4">
                    <div className="space-y-2.5">
                      {group.published.fields
                        ...
                        .sort((a, b) => a.sortOrder - b.sortOrder)
                        .map((field) => {
                          const staged = contentEdits[group.published!.id]?.[field.key]
                          return (
                            <div
                              key={field.id}
                              className={cn(
                            ...
                            >
                              <div className="min-w-0 space-y-1">
                                <Input
                                  ...
                                  onChange={(event) =>
                                    stageContent(group.published!.id, field.key, { label: event.target.value })
                                  }
                                />
                                <p className="truncate text-[11px] text-[#9aa7b8]">
                              ...
                              <Input
                                ...
                                onChange={(event) =>
                                  stageContent(group.published!.id, field.key, { helpText: event.target.value })
                                }
                              />
                                ...
                                {field.isCore ? (
                                  ...
                                  <Switch
                                    ...
                                    onCheckedChange={(active) =>
                                      void toggleFieldActive(group.published!, field, active)
                                    }
                                  />
                                )}
                              </div>
                    ...
                    <div className="mt-3 flex justify-end">
                      <Button
                        size="sm"
                        disabled={!contentEdits[group.published.id] || busy === `content-${group.published.id}`}
                ...
                {group.draft && isSuperadmin ? (
                  <div className="mt-5 rounded-[0.35rem] border border-dashed border-[#7cb3ff] bg-[#f8fbff] p-4">
                    <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[#005db6]">
                      Draft v{group.draft.version} · structural editor
                    </p>
                    <div className="mt-3 space-y-2">
                      {(draftEdits[group.draft.id] ?? []).map((field, index) => (
                        <div
                        ...
                        >
                          <Input
                            ...
                            onChange={(event) =>
                              setDraftEdits((prev) => ({
                                ...prev,
                                [group.draft!.id]: prev[group.draft!.id].map((entry, i) =>
                                  i === index
                          ...
                          <Input
                            ...
                            onChange={(event) =>
                              setDraftEdits((prev) => ({
                                ...prev,
                                [group.draft!.id]: prev[group.draft!.id].map((entry, i) =>
                                  i === index ? { ...entry, section: event.target.value } : entry,
                                ),
                              }))
                            }
                          />
                          ...
                          <Select
                            ...
                            onValueChange={(type) =>
                              setDraftEdits((prev) => ({
                                ...prev,
                                [group.draft!.id]: prev[group.draft!.id].map((entry, i) =>
                                  i === index ? { ...entry, type: type as EvaluationFieldType } : entry,
                                ),
                              }))
                            }
                          >
                            ...
                            <Switch
                              ...
                              onCheckedChange={(required) =>
                                setDraftEdits((prev) => ({
                                  ...prev,
                                  [group.draft!.id]: prev[group.draft!.id].map((entry, i) =>
                                    i === index ? { ...entry, required } : entry,
                                  ),
                                }))
                              }
                            />
                            <span className="text-[11px] text-[#74777f]">req</span>
                          </div>
                          <Button
                            variant="ghost"
                            size="icon"
                            ...
                            onClick={() =>
                              setDraftEdits((prev) => ({
                                ...prev,
                                [group.draft!.id]: prev[group.draft!.id].filter((_, i) => i !== index),
                              }))
                            }
                          >
                            {field.isCore ? (
                    ...
                    <div className="mt-3 flex flex-wrap justify-between gap-2">
                          ...
                          setDraftEdits((prev) => ({
                            ...
                            [group.draft!.id]: [
                              ...(prev[group.draft!.id] ?? []),
                              {
                                ...
                                options: null,
                                required: false,
                                sortOrder: ((prev[group.draft!.id]?.length ?? 0) + 1) * 10,
                                active: true,
                                isCore: false,
                              },
                            ],
                      ...
                      <div className="flex gap-2">
                        ...
                        <Button
                          ...
                          onClick={() =>
                            void run(
                              ...
                              async () => {
                                await publishEvaluationForm(client, group.draft!.id)
                                toast.success(`Version ${group.draft!.version} published.`)
                                await load()
                              },
                        ...
                        >
                          <Send className="mr-1.5 h-4 w-4" /> Publish v{group.draft.version}
                        </Button>
                      </div>
                    </div>
                  </div>
