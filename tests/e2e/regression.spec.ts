...
test.describe('Regression & stability (storageState: superadmin)', () => {
  ...
  test('two tabs in one session both stay authenticated', async ({ context }) => {
    ...
    await expect(p1.getByRole('button', { name: 'Notifications' }).first()).toBeVisible({ timeout: 20_000 })
    await expect(p2.getByRole('button', { name: 'Notifications' }).first()).toBeVisible({ timeout: 20_000 })
