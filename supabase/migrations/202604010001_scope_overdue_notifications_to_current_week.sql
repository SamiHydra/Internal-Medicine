...
declare
  ...
  v_current_deadline_at timestamptz;
begin
  select period.id, period.deadline_at
  into v_current_period_id, v_current_deadline_at
  ...
  if v_current_period_id is null or v_current_deadline_at >= timezone('utc', now()) then
    return;
  end if;
  ...
  for v_item in
    ...
    join public.reporting_periods period
      on period.id = v_current_period_id
